# 6_theoretical_limits.py (JAX)
import jax
import jax.numpy as jnp

from frameworks.jax.utils import DTYPE, init_linear, linear, normal, softmax, viz_stage
...
def fisher_information_matrix(params, x):
    def log_likelihood(p):
        output = linear(p, x)
        return -jnp.sum(output**2) / 2

    gradients = jax.grad(log_likelihood)(params)
    grad_vector = jnp.concatenate([gradients["weight"].flatten(), gradients["bias"].flatten()])
    return jnp.outer(grad_vector, grad_vector)
...
fisher_max = jnp.max(fisher_matrix)
fisher_min = jnp.min(fisher_matrix)
...
def estimate_kolmogorov_complexity(data):
    ...
    threshold = jnp.mean(data_flat)
    data_binary = (data_flat > threshold).astype(jnp.int32)
    ...
    def entropy_compress(binary_data):
        ...
        ones = jnp.sum(binary_data.astype(DTYPE))
        zeros = total - ones

        if ones > 0 and zeros > 0:
            p_ones = ones / total
            p_zeros = zeros / total
            entropy = -(p_ones * jnp.log2(p_ones) + p_zeros * jnp.log2(p_zeros))
        else:
            entropy = jnp.array(0.0, dtype=DTYPE)
...
structured_data = jnp.sin(jnp.arange(0, 4 * jnp.pi, 4 * jnp.pi / 100, dtype=DTYPE))
...
def quantum_superposition_layer(x, n_qubits=4):
    ...
    amplitude_norms = jnp.sqrt(real_amplitudes**2 + imag_amplitudes**2)
    total_norm = jnp.sum(amplitude_norms, axis=1, keepdims=True)
    ...
    qubit_indices = jnp.arange(n_qubits, dtype=DTYPE).reshape(1, -1)
    classical_output = jnp.sum(probabilities * qubit_indices, axis=1)
...
def compute_phi(network_state, connectivity_matrix):
    ...
    system_entropy = -jnp.sum(network_state * jnp.log2(network_state + 1e-8))

    part_entropies = jnp.array(0.0, dtype=DTYPE)
    for i in range(n_nodes):
        node_prob = network_state[i]
        if node_prob > 0 and node_prob < 1:
            node_entropy = -(node_prob * jnp.log2(node_prob) + (1 - node_prob) * jnp.log2(1 - node_prob))
            part_entropies = part_entropies + node_entropy

    phi = system_entropy - part_entropies
    connectivity_strength = jnp.sum(jnp.abs(connectivity_matrix))
...
disconnected_matrix = jnp.eye(6, dtype=DTYPE) * 0.1
...
def infinite_width_approximation(target_func, x_points, width_schedule):
    ...
    for width in width_schedule:
        ...
        h = jnp.tanh(linear(params["l1"], x_points.reshape(-1, 1)))
        network_output = linear(params["l2"], h)
        target_output = target_func(x_points).reshape(-1, 1)

        error = jnp.mean((network_output - target_output) ** 2)
...
def complex_target(x):
    return jnp.sin(3 * x) * jnp.exp(-(x**2)) + jnp.cos(5 * x) * x


x_test = jnp.arange(-2, 2, 4 / 100, dtype=DTYPE)
...
def compute_learning_thermodynamics(params, data, learning_rate):
    ...
    gradients = jax.grad(loss_fn)(params, data)
    initial_energy = jnp.sum(params["weight"] ** 2) + jnp.sum(params["bias"] ** 2)

    updated_weight = params["weight"] - learning_rate * gradients["weight"]
    updated_bias = params["bias"] - learning_rate * gradients["bias"]
    updated_energy = jnp.sum(updated_weight**2) + jnp.sum(updated_bias**2)
