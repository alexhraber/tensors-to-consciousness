# 2_optimization_theory.py (JAX)
import jax
import jax.numpy as jnp

from frameworks.jax.utils import DTYPE, viz_stage
...
def gradient_descent_step(x, learning_rate):
    grad = jax.grad(quadratic_bowl)(x)
    return x - learning_rate * grad


x = jnp.array(10.0, dtype=DTYPE)
learning_rate = 0.1
history = [float(x)]
...
def gradient_descent_with_momentum(func, x_start, lr, momentum, steps):
    x0 = x_start
    velocity = jnp.array(0.0, dtype=DTYPE)
    hist = [float(x0)]
...
final_vanilla, hist_vanilla = gradient_descent_with_momentum(
    quadratic_bowl, jnp.array(10.0, dtype=DTYPE), 0.05, 0.0, 50
)
final_momentum, hist_momentum = gradient_descent_with_momentum(
    quadratic_bowl, jnp.array(10.0, dtype=DTYPE), 0.05, 0.9, 50
)
...
def simple_adam_step(x0, grad, m, v, t, lr=0.001, beta1=0.9, beta2=0.999, eps=1e-8):
    ...
    m_hat = m / (1 - beta1**t)
    v_hat = v / (1 - beta2**t)
    x0 = x0 - lr * m_hat / (jnp.sqrt(v_hat) + eps)
    return x0, m, v


x = jnp.array(10.0, dtype=DTYPE)
m = jnp.array(0.0, dtype=DTYPE)
