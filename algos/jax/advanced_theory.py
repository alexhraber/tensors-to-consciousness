# 4_advanced_theory.py (JAX)
import jax
import jax.numpy as jnp

from frameworks.jax.utils import DTYPE, init_linear, linear, normal, softmax, uniform, viz_stage
...
def generate_swiss_roll(n_samples):
    t = uniform(0, 4 * jnp.pi, (n_samples,), dtype=DTYPE)
...
class InformationBottleneck:
    ...
    def __call__(self, x):
        z = jnp.tanh(linear(self.encoder, x))
...
def attention_mechanism(queries, keys, values, temperature=1.0):
    scores = jnp.matmul(queries, keys.T) / jnp.sqrt(jnp.array(queries.shape[-1], dtype=DTYPE))
...
def riemannian_gradient_step(x, grad, lr=0.01):
    tangent_grad = grad - jnp.sum(grad * x) * x
    x_new = x - lr * tangent_grad
    return project_to_sphere(x_new)


def sphere_objective(x):
    target = jnp.array([1.0, 0.0, 0.0], dtype=DTYPE)
    return jnp.sum((x - target) ** 2)


x = project_to_sphere(normal((3,), dtype=DTYPE))
print("Optimizing on sphere manifold:")
print(f"Initial: obj={sphere_objective(x):.6f}, ||x||={jnp.sqrt(jnp.sum(x**2)):.6f}")

for i in range(10):
    grad = jax.grad(sphere_objective)(x)
    x = riemannian_gradient_step(x, grad)
    if i % 3 == 0:
        obj_val = sphere_objective(x)
        constraint_violation = jnp.abs(jnp.sum(x**2) - 1.0)
