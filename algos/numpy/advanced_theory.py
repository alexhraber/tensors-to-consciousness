# 4_advanced_theory.py (NumPy)
import numpy as np

from frameworks.numpy.utils import DTYPE, finite_diff_grad_vector, init_linear, linear, normal, softmax, uniform, viz_stage

VIZ_META = {}

print("🔬 ADVANCED COMPUTATIONAL THEORY (NumPy)")
print("=" * 50)
...
print(f"Data range: [{np.min(swiss_data):.2f}, {np.max(swiss_data):.2f}]")
...
class InformationBottleneck:
    ...
    def __call__(self, x):
        z = np.tanh(linear(self.encoder, x))
...
for name, model in models.items():
    ...
    bottleneck_info = np.std(bottleneck)
...
def attention_mechanism(queries, keys, values, temperature=1.0):
    scores = np.matmul(queries, keys.T) / np.sqrt(np.array(queries.shape[-1], dtype=DTYPE))
...
print(f"Attention weights (row sums): {np.sum(weights, axis=1)}")
print(f"Max attention per query: {np.max(weights, axis=1)}")
...
def sphere_objective(x):
    target = np.array([1.0, 0.0, 0.0], dtype=DTYPE)
...
for i in range(10):
    ...
    if i % 3 == 0:
        ...
        constraint_violation = np.abs(np.sum(x**2) - 1.0)
