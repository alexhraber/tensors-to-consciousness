...
from frameworks.numpy.utils import DTYPE, init_linear, linear, normal, viz_stage
...
print(f"Base output norm: {np.sqrt(np.sum(base_output**2)):.4f}")
...
def find_lottery_ticket(model, pruning_ratio=0.9):
    ...
    sorted_weights = np.sort(np.abs(weight_vector))
    threshold_idx = int(len(sorted_weights) * pruning_ratio)
    threshold = sorted_weights[threshold_idx]

    mask = np.abs(weight_vector) > threshold
    sparsity = 1.0 - np.mean(mask.astype(DTYPE))

    print(f"Lottery ticket sparsity: {sparsity:.1%}")
    print(f"Remaining parameters: {np.sum(mask)} / {len(weight_vector)}")
    return mask
...
def simulate_grokking_dynamics(steps=1000):
    ...
    for step in range(steps):
        if step < memorization_phase:
            train_loss = 2.0 * np.exp(np.array(-step / 100, dtype=DTYPE))
            test_loss = np.array(2.0, dtype=DTYPE) + 0.1 * normal((), dtype=DTYPE)
            ...
            train_loss = np.array(prev_train, dtype=DTYPE) + 0.1 * normal((), dtype=DTYPE)
            test_loss = np.array(prev_test, dtype=DTYPE) + 0.2 * normal((), dtype=DTYPE)
        else:
            progress = (step - generalization_phase) / (steps - generalization_phase)
            train_loss = np.array(0.1, dtype=DTYPE) + 0.05 * np.exp(np.array(-progress * 5, dtype=DTYPE))
            test_loss = np.array(0.1, dtype=DTYPE) + 0.05 * np.exp(np.array(-progress * 5, dtype=DTYPE))
...
def model_capability_curve(model_sizes, task_complexity):
    ...
    for size in model_sizes:
        ...
        capability = 1 / (1 + np.exp(np.array(exponent, dtype=DTYPE)))
