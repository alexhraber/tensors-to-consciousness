# 2_optimization_theory.py (NumPy)
import numpy as np

from frameworks.numpy.utils import DTYPE, finite_diff_grad_scalar, viz_stage

VIZ_META = {}

print("🎯 OPTIMIZATION THEORY (NumPy)")
print("=" * 50)
...
def gradient_descent_with_momentum(func, x_start, lr, momentum, steps):
    x0 = x_start
    velocity = np.array(0.0, dtype=DTYPE)
    hist = [float(x0)]
...
final_vanilla, hist_vanilla = gradient_descent_with_momentum(
    quadratic_bowl, np.array(10.0, dtype=DTYPE), 0.05, 0.0, 50
)
final_momentum, hist_momentum = gradient_descent_with_momentum(
    quadratic_bowl, np.array(10.0, dtype=DTYPE), 0.05, 0.9, 50
)
