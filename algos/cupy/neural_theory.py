# 3_neural_theory.py (CuPy)
import cupy as cp

from frameworks.cupy.utils import DTYPE, finite_diff_grad_dict, gelu, init_linear, linear, normal, relu, sigmoid, tree_l2_norm, uniform, viz_stage

VIZ_META = {}

print("🧠 NEURAL NETWORK THEORY (CuPy)")
print("=" * 50)
...
def target_function(x):
    return cp.sin(2 * cp.pi * x) + 0.5 * cp.cos(4 * cp.pi * x)
...
def analyze_layer_activations(model, x):
    ...
    print(f"Layer 1 (linear): mean={cp.mean(current):.4f}, std={cp.std(current):.4f}")
    current = relu(current)
    print(f"Layer 1 (ReLU):   mean={cp.mean(current):.4f}, std={cp.std(current):.4f}")
    activations.append(current)

    current = linear(model.params["l2"], current)
    print(f"Layer 2 (linear): mean={cp.mean(current):.4f}, std={cp.std(current):.4f}")
    current = relu(current)
    print(f"Layer 2 (ReLU):   mean={cp.mean(current):.4f}, std={cp.std(current):.4f}")
    activations.append(current)

    current = linear(model.params["l3"], current)
    print(f"Layer 3 (linear): mean={cp.mean(current):.4f}, std={cp.std(current):.4f}")
    activations.append(current)
...
def loss_function(flat_params, x, y):
    ...
    return cp.mean((predictions - y) ** 2)
...
def compare_activations(x):
    ...
    for name, activation in activations.items():
        mean_val = cp.mean(activation)
        std_val = cp.std(activation)
