# 6_theoretical_limits.py (CuPy)
import cupy as cp

from frameworks.cupy.utils import DTYPE, finite_diff_grad_dict, init_linear, linear, normal, scalar, softmax, viz_stage
...
def kl_divergence(p, q):
    eps = 1e-8
    p_safe = cp.maximum(p, eps)
    q_safe = cp.maximum(q, eps)
    return cp.sum(p_safe * cp.log(p_safe / q_safe))


def fisher_information_matrix(params, x):
    def log_likelihood(p):
        output = linear(p, x)
        return -cp.sum(output**2) / 2

    gradients = finite_diff_grad_dict(log_likelihood, params)
    grad_vector = cp.concatenate([gradients["weight"].flatten(), gradients["bias"].flatten()])
    return cp.outer(grad_vector, grad_vector)
...
fisher_max = cp.max(fisher_matrix)
fisher_min = cp.min(fisher_matrix)
condition_number = fisher_max / (cp.abs(fisher_min) + 1e-8)
...
def estimate_kolmogorov_complexity(data):
    ...
    threshold = cp.mean(data_flat)
    data_binary = (data_flat > threshold).astype(cp.int32)
    ...
    def entropy_compress(binary_data):
        ...
        ones = cp.sum(binary_data.astype(DTYPE))
        zeros = total - ones

        if scalar(ones) > 0 and scalar(zeros) > 0:
            p_ones = ones / total
            p_zeros = zeros / total
            entropy = -(p_ones * cp.log2(p_ones) + p_zeros * cp.log2(p_zeros))
        else:
            entropy = cp.array(0.0, dtype=DTYPE)
...
structured_data = cp.sin(cp.arange(0, 4 * cp.pi, 4 * cp.pi / 100, dtype=DTYPE))
...
def quantum_superposition_layer(x, n_qubits=4):
    ...
    amplitude_norms = cp.sqrt(real_amplitudes**2 + imag_amplitudes**2)
    total_norm = cp.sum(amplitude_norms, axis=1, keepdims=True)
    ...
    qubit_indices = cp.arange(n_qubits, dtype=DTYPE).reshape(1, -1)
    classical_output = cp.sum(probabilities * qubit_indices, axis=1)
...
print(f"Probability conservation check: {cp.sum(quantum_probs, axis=1)}")
print(f"Quantum coherence measure: {cp.std(quantum_probs):.4f}")
...
def compute_phi(network_state, connectivity_matrix):
    ...
    system_entropy = -cp.sum(network_state * cp.log2(network_state + 1e-8))

    part_entropies = cp.array(0.0, dtype=DTYPE)
    for i in range(n_nodes):
        node_prob = network_state[i]
        node_prob_val = scalar(node_prob)
        if node_prob_val > 0 and node_prob_val < 1:
            node_entropy = -(node_prob * cp.log2(node_prob) + (1 - node_prob) * cp.log2(1 - node_prob))
            part_entropies = part_entropies + node_entropy

    phi = system_entropy - part_entropies
    connectivity_strength = cp.sum(cp.abs(connectivity_matrix))
...
disconnected_matrix = cp.eye(6, dtype=DTYPE) * 0.1
...
def infinite_width_approximation(target_func, x_points, width_schedule):
    ...
    for width in width_schedule:
        ...
        h = cp.tanh(linear(params["l1"], x_points.reshape(-1, 1)))
        network_output = linear(params["l2"], h)
        target_output = target_func(x_points).reshape(-1, 1)

        error = cp.mean((network_output - target_output) ** 2)
...
def complex_target(x):
    return cp.sin(3 * x) * cp.exp(-(x**2)) + cp.cos(5 * x) * x


x_test = cp.arange(-2, 2, 4 / 100, dtype=DTYPE)
