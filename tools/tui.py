#!/usr/bin/env python3
"""Interactive terminal UI for explorer rendering."""
...
def _quick_edit_state(fd: int, state: shinkei.RenderState, old: list[int]) -> None:
    ...
    try:
        ...
        tty.setcbreak(fd)
    parsed = core.parse_assignment(line)
...
def _command_console(
    ...
    while True:
        ...
        if cmd == "set":
            ...
            parsed = core.parse_assignment(expr)
            if parsed is not None:
                key, value = parsed
            else:
                if "=" not in expr:
...
def _draw_text_frame(frame_text: str, prev_frame_text: str | None, telemetry: dict[str, int] | None = None) -> str:
    patch = core.frame_patch(prev_frame_text or "", frame_text)
