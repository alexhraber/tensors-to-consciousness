...
class ShinkeiRenderTests(unittest.TestCase):
    def test_ascii_heatmap_shape(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        arr = np.arange(100, dtype=np.float32).reshape(10, 10)
        out = shinkei_render._ascii_heatmap(arr, width=8, height=4)
    ...
    def test_ascii_heatmap_empty(self) -> None:
        ...
        self.assertEqual(shinkei_render._ascii_heatmap(arr), "(empty)")

    def test_half_block_render_constant_tensor_still_visible(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        arr = np.ones((8, 8), dtype=np.float32)
        out = shinkei_render._pixel_heatmap(arr, width=8, height=4)
        self.assertTrue(any(ch.strip() for ch in out.splitlines()))

    def test_render_stage_filters_and_limit(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        ...
        with redirect_stdout(buf):
            shinkei_render.render_stage("stage_x", scope, to_numpy, framework="numpy", limit=2)
        out = buf.getvalue()
        self.assertIn("[RENDER:numpy] stage_x", out)
    ...
    def test_render_stage_respects_env_toggle(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        scope = {"x": np.ones((4, 4), dtype=np.float32)}

        with patch.dict(os.environ, {"RENDER": "0"}):
            buf = io.StringIO()
            with redirect_stdout(buf):
                shinkei_render.render_stage("stage_off", scope, lambda x: x, framework="numpy")
        self.assertEqual(buf.getvalue(), "")

    def test_render_stage_renders_metadata_line(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        scope = {"x": np.arange(16, dtype=np.float32).reshape(4, 4)}
        buf = io.StringIO()
        with patch.object(shinkei_render, "_supports_graphical_terminal", return_value=False):
            with redirect_stdout(buf):
                shinkei_render.render_stage(
                    "stage_meta",
                    scope,
                    lambda x: x,
                    framework="numpy",
    ...
    def test_render_stage_plots_falls_back_to_heatmap(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        ...
        with patch.dict(
            os.environ,
            {"RENDER_STYLE": "plots", "RENDER_TRACE": "1"},
            clear=False,
            ...
            buf = io.StringIO()
            with redirect_stdout(buf):
                shinkei_render.render_stage("stage_fallback", scope, lambda x: x, framework="numpy")
        self.assertIn("[RENDER renderer=heatmap]", buf.getvalue())

    def test_render_stage_default_prefers_plots_when_inline_available(self) -> None:
        shinkei_render = _load_shinkei_render()
        np = shinkei_render._np_module()
        scope = {"x": np.arange(64, dtype=np.float32).reshape(8, 8)}

        with patch.dict(os.environ, {"RENDER_TRACE": "1"}, clear=False), patch.object(
            ...
            buf = io.StringIO()
            with redirect_stdout(buf):
                shinkei_render.render_stage("stage_fluid", scope, lambda x: x, framework="numpy")
        self.assertIn("[RENDER renderer=plots]", buf.getvalue())
    ...
    def test_inline_support_requires_capability_or_force(self) -> None:
        ...
        with patch.dict(
            ...
            self.assertFalse(shinkei_render._supports_inline_image_graphics())

        with patch.dict(
            os.environ,
            {"RENDER_FORCE_INLINE": "1"},
            clear=False,
        ), patch.object(shinkei_render.sys.stdout, "isatty", return_value=True):
